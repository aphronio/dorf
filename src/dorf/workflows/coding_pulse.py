...
def build_coding_job_pulse(
    ...
    terminal = coding_job.status if coding_job.status in AFK_TERMINAL_JOB_STATUSES else None
    if terminal is None and inspection.job.status == "ended":
        terminal = "ended"

    outcome_stage = _outcome_stage(coding_job.status, coding_job.metadata, terminal)
    latest_delta = _latest_delta(coding_job, events, terminal)
    ...
    attention = _attention(inspection, outcome_stage=outcome_stage, terminal=terminal)
    timestamps = [
...
def _observed_activity(
    ...
    *,
    ...
    if turn is not None and turn.status == "running":
        support = (
            "consistent"
            if worker_claim is not None and worker_claim.assignment_id == inspection.assignment.id
            else "unconfirmed"
...
def _attention(
    ...
    *,
    ...
    turn = inspection.latest_turn
    if turn is not None and turn.status in {"failed", "recovery-required"}:
