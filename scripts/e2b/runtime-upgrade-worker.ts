...
const [provider, operation, job] = process.argv.slice(2);
if (!['incus', 'e2b'].includes(provider)) throw new Error('provider must be incus or e2b');
if (operation && (!['probe', 'cleanup-unstarted'].includes(operation) || !/^job-[a-f0-9]{20}$/.test(job ?? ''))) {
  throw new Error('optional operation: probe JOB | cleanup-unstarted JOB');
}
const child = Bun.spawn(['mise', 'exec', '--', 'go', 'test', './internal/postgres',
  ...
  env: { ...process.env, DORF_LIVE_UPGRADE_PROVIDER: provider, GOFLAGS: '-p=1',
    DORF_UPGRADE_PROBE_JOB: job ?? '', DORF_UPGRADE_PROBE_ACTION: operation === 'cleanup-unstarted' ? 'cleanup' : '' },
