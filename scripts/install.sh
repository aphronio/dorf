...
cleanup() {
  ...
  [ -z "$remote_incus_temp" ] || rm -f -- "$remote_incus_temp"
  rm -rf -- "$work_dir"
}
trap cleanup EXIT
...
[ -f "$work_dir/extract/bootstrap/incus-remote.sh" ] ||
  fail "release archive does not contain bootstrap/incus-remote.sh"
chmod 0755 "$work_dir/extract/dorf" "$work_dir/extract/bootstrap/incus-remote.sh"
...
remote_incus_target="$install_dir/dorf-incus-remote.sh"
...
[ ! -d "$remote_incus_target" ] || fail "$remote_incus_target is a directory"
...
incus_compose_temp="$(mktemp "$install_dir/.dorf-compose-incus.install.XXXXXX")" ||
  fail "cannot write to $install_dir; pass --install-dir ABSOLUTE_DIR"
remote_incus_temp="$(mktemp "$install_dir/.dorf-incus-remote.install.XXXXXX")" ||
  fail "cannot write to $install_dir; pass --install-dir ABSOLUTE_DIR"
...
install -m 0755 "$work_dir/extract/bootstrap/incus-remote.sh" "$remote_incus_temp"
mv -f -- "$compose_temp" "$compose_target"
compose_temp=""
mv -f -- "$incus_compose_temp" "$incus_compose_target"
incus_compose_temp=""
mv -f -- "$remote_incus_temp" "$remote_incus_target"
remote_incus_temp=""
