...
func (e *idleReaderExecution) ReconcileIdleSandboxes(_ context.Context, sessionID string) error {
	if e.store.inFence {
		panic("idle reconciliation nested the reader fence")
	}
	if sessionID != e.store.session.ID {
		panic("idle reconciliation changed Session")
	}
	e.calls++
...
func TestFileReadReconcilesIdleAfterFenceWithoutLosingResult(t *testing.T) {
	...
	service := Service{Store: store, Runtimes: readerTestRuntimes{profile: session.SandboxProfile, files: &readerTestFiles{contents: []byte("retained result")}, execution: execution}}
