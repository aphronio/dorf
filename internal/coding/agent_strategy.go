...
// ReviewAgentStrategy is coding's cohesive strict-review Harness adapter.
// Core owns AgentRun preparation, recovery, submission ordering, observation,
// binding, and fencing; this strategy owns only the exact review boundary.
type ReviewAgentStrategy struct {
	store     Store
	externals Externals
	job       Job
	run       ReviewRunView
...
func NewReviewAgentStrategy(ctx context.Context, store Store, externals Externals, execution core.AgentMessageExecution) (ReviewAgentStrategy, error) {
	job, err := store.CodingJob(ctx, execution.Job.ID)
	if err != nil {
		return ReviewAgentStrategy{}, err
	}
	run, err := store.ReviewRun(ctx, core.AgentRunID(execution.Message.ID))
	if err != nil {
		return ReviewAgentStrategy{}, err
	}
	strategy := ReviewAgentStrategy{store: store, externals: externals, job: job, run: run}
	if err := strategy.validate(ctx, execution); err != nil {
		return ReviewAgentStrategy{}, err
	}
	return strategy, nil
}

func (s ReviewAgentStrategy) Harness() string { return s.externals.Harness() }

func (s ReviewAgentStrategy) validate(ctx context.Context, execution core.AgentMessageExecution) error {
	run := s.run
...
func (s ReviewAgentStrategy) attempt(run core.AgentRun) ReviewRunView {
	return reviewRunAttempt(run, s.run.Request, s.run.Sandbox)
}

func (s ReviewAgentStrategy) SubmitNew(ctx context.Context, run core.AgentRun) (core.HarnessBinding, error) {
	return s.externals.ReviewInitialTurn(ctx, s.job, s.attempt(run))
}

func (s ReviewAgentStrategy) Recover(ctx context.Context, run core.AgentRun) (core.HarnessBinding, error) {
	return s.externals.ReviewRecover(ctx, s.job, s.attempt(run))
}

func (s ReviewAgentStrategy) History(ctx context.Context, run core.AgentRun) (core.HarnessHistory, error) {
	return s.externals.ReviewTurns(ctx, s.job, s.attempt(run))
}

var _ core.AgentHarnessStrategy = ReviewAgentStrategy{}
