...
func TestReviewRequestIdentityIsStableAndRevisionScoped(t *testing.T) {
	jobID := core.JobID("client-request-40")
	revisionA := strings.Repeat("a", 40)
	revisionB := strings.Repeat("b", 40)
	role := "critical-boundary"
	if ReviewRequestMessageID(jobID, revisionA, role) != core.MessageID(jobID, core.MessageFromWorkflow, ReviewRequestFromID(revisionA, role)) ||
