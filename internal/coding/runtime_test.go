...
func TestTaskAndWakeIdentitiesRemainStable(t *testing.T) {
	...
	if stepName != "dorf/agent-run-wake/v2/message-1/00000000000000000002" || timeout != time.Second {
	...
	if stepName != "dorf/proposal-wake/v3/rev-1/00000000000000000003" || timeout != 30*time.Second {
		t.Fatalf("proposal wake=%q %s", stepName, timeout)
	}
	stepName, timeout = wakeOptions(Work{Kind: WorkAttention}, 4, 30*time.Second)
	if stepName != "dorf/job-execution-wake/v1/00000000000000000004" || timeout != idleMessagePollInterval {
