...
func TestLifecycleReconcilesLostCreateResponseAndDeletesOnlyOwnedSandbox(t *testing.T) {
	...
	owner := Ownership{SessionID: "session-1", SandboxID: "dorf-session-1", OwnershipNonce: strings.Repeat("a", 64)}
	client := Client{
	...
	adapter := Adapter{Client: client, Config: AdapterConfig{SandboxTimeout: 10 * time.Minute, Workspace: "/workspace"}}
...
func TestAdapterEndpointRequiresOwnershipAndBuildsScopedProviderDial(t *testing.T) {
	...
	owner := Ownership{SessionID: "session-endpoint", SandboxID: "dorf-session-endpoint", OwnershipNonce: strings.Repeat("e", 64)}
...
func TestCreateCanExplicitlyUseProfileInternetAccess(t *testing.T) {
	api := newFakeAPI(t)
	client := Client{APIURL: "https://e2b.test", APIKey: "test-key", HTTPClient: &http.Client{Transport: handlerTransport{handler: api}}}
	owner := Ownership{SessionID: "session-internet", SandboxID: "dorf-session-internet", OwnershipNonce: strings.Repeat("d", 64)}
...
func TestCreateClassifiesMissingTemplateAsUnavailableProfileArtifact(t *testing.T) {
	...
	owner := Ownership{SessionID: "session-missing", SandboxID: "sandbox-missing", OwnershipNonce: strings.Repeat("f", 64)}
...
func TestFindOwnedPaginatesRunningAndPausedAndRejectsDuplicates(t *testing.T) {
	owner := Ownership{SessionID: "session-2", SandboxID: "dorf-session-2", OwnershipNonce: strings.Repeat("c", 64)}
	requests := 0
	handler := http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		requests++
