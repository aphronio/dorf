...
func newHandlerContext(discovery Discovery, auth Auth, sessions Sessions, profiles Profiles, shutdown context.Context) http.Handler {
	...
	h.mux.HandleFunc("/v1/sessions/{session}/input-capabilities", h.authenticate(h.inputCapabilitiesRoute))
	...
	h.mux.HandleFunc("/v1/sessions/{session}/checkpoints", h.authenticate(h.checkpointStartRoute))
	h.mux.HandleFunc("/v1/checkpoints/{checkpoint}", h.authenticate(h.checkpointRoute))
	h.mux.HandleFunc("/v1/sessions/{session}/activate", h.authenticate(h.activateSessionRoute))
