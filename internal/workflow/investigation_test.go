...
func TestCodebaseInvestigationProjectsItsOwnDependencyChain(t *testing.T) {
	...
	job := core.Job{ID: "job-1", Workflow: investigation.Workflow, WorkflowRevision: investigation.WorkflowRevision, AdmissionOpen: true}
	...
	snapshot := InvestigationSnapshot{Job: job, MainSandbox: sandbox, Deliveries: []core.Delivery{delivery}, Delivery: delivery, Source: investigation.Source{JobID: job.ID, Kind: investigation.SourceRemote, Repository: "https://example.test/repo.git", Revision: revision}}
...
func TestCodebaseInvestigationProjectsRetainedBundleRestore(t *testing.T) {
	...
	job := core.Job{ID: "job-local", Workflow: investigation.Workflow, WorkflowRevision: investigation.WorkflowRevision, AdmissionOpen: true}
	sandbox := core.Sandbox{ID: core.MainSandboxName(job.ID), JobID: job.ID}
