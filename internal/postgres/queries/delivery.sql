...
with current_turn_start as (
    ...
    select m.id as message_id,m.sequence
    from dorf.job_messages m join dorf.agent_runs ar on ar.message_id=m.id
    where m.job_id=sqlc.arg(job_id) and m.delivery_intent='steer'
...
select m.id,m.job_id,m.from_kind,m.from_id,m.sequence,m.input,m.attachments,m.delivery_intent,
       coalesce(m.steer_target_turn_id,'') as steer_target_turn_id,m.admitted_at,m.refresh_skills,m.developer_instructions,m.observation
