-- name: NextDeliveryCandidate :one
with current_turn_start as (
    ...
    where m.job_id=sqlc.arg(job_id) and ar.native_turn_id is not null
    ...
    from dorf.job_messages m join dorf.agent_runs ar on ar.message_id=m.id
    where m.job_id=sqlc.arg(job_id) and ar.native_turn_id is null and not exists(select 1 from current_turn_start)
)
