...
where s.id=r.sandbox_id and s.id=sqlc.arg(sandbox_id) and s.session_id=sqlc.arg(session_id)
...
where s.session_id=sqlc.arg(session_id) and s.id=sqlc.arg(sandbox_id) and r.id=sqlc.arg(resource_id);
