...
with reserved as (
    ...
    values(sqlc.arg(id),sqlc.arg(session_id),sqlc.arg(name),sqlc.arg(id)::text || ':initial')
...
from dorf.actions
where session_id=sqlc.arg(session_id) and kind=sqlc.arg(kind) and scope_key=sqlc.arg(scope_key);

-- name: InsertScopedAction :execrows
insert into dorf.actions(id,session_id,kind,state,scope_key)
values(sqlc.arg(id),sqlc.arg(session_id),sqlc.arg(kind),'unsettled',sqlc.arg(scope_key))
...
from dorf.actions a
where a.session_id=sqlc.arg(session_id)
