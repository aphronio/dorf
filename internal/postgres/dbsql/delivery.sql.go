...
with current_turn_start as (
    ...
    where m.session_id=$1 and ar.turn_id is not null
    ...
    from dorf.session_messages m join dorf.agent_runs ar on ar.message_id=m.id
    where m.session_id=$1 and m.delivery_intent='follow'
      and ar.turn_id is null and ar.state in ('submitting','uncertain')
    order by m.sequence limit 1
), unsettled_steer as (
    select m.id as message_id,m.sequence
    from dorf.session_messages m join dorf.agent_runs ar on ar.message_id=m.id
    where m.session_id=$1 and m.delivery_intent='steer'
...
func (q *Queries) NextAgentMessage(ctx context.Context, sessionID string) (NextAgentMessageRow, error) {
	row := q.db.QueryRowContext(ctx, nextAgentMessage, sessionID)
