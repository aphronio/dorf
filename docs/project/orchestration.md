...
- Dorf inspection owns the coding dependency chain and chronological Job, Message, Action, Check,
  AgentRun, Revision, Proposal, Outcome, and cleanup facts. Absurd inspection owns task execution and
  recovery mechanics.
...
Use Dorf inspection for the full product workflow, current work, and chronological product facts.
Use `absurdctl list-tasks` and `absurdctl dump-task` for
...
2. Confirm whether an existing Job, Sandbox, Thread, branch, or pull request already owns the work.
...
8. Apply deterministic review policy and return reviewer text through an ordinary Message to the
   implementation AgentRun path and continuing Thread.
...
2. inspect the existing Dorf Job, Sandbox, Thread, Absurd task, and unsettled Actions;
