...
2. Use the installer from an immutable release, or the manual attested-release fallback documented
   ...
   A standalone install prints the setup handoff; use `dorf update` for later upgrades through the
   same verified installer path without repeating that fresh-install hint.
