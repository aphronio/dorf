...
| --- | --- | --- |
| Deployment-host agent | Install Dorf, run setup, use documented Compose and Session operations, manage Profiles, and run diagnostics. | Pause for each secret, browser authorization, paid service, administrator helper, DNS replacement, or consequential infrastructure choice. |
| Remote-client agent | Install and use the Dorf CLI, connect one Client, and use documented Session operations. | Do not run deployment-host commands, access PostgreSQL, operate the host Compose project, or use SSH or local commands to bypass a missing remote capability. |

Do not combine the roles unless the human explicitly grants both. The deployment host retains its
own credentials and infrastructure authority when a remote-client agent operates Sessions.
...
| Run a direct Session on the deployment host | [Run a direct Session](getting-started.md#4-run-a-direct-session-on-the-deployment-host) |
| Diagnose installation, readiness, authentication, Session, or cleanup failure | [Support and diagnostics](support.md) |
...
credential, Gateway state, environment dump, Harness transcript, or unrequested Session output. Let the
...
- The requested Session identity, current state, and requested result.
